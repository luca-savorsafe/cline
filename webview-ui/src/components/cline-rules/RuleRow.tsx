			...
			<div className="flex items-center px-2 py-4 rounded bg-text-block-background max-h-4">
				<span className="flex-1 overflow-hidden break-all whitespace-normal flex items-center mr-1" title={rulePath}>
					...
					{ruleType === "agents" && (
						<Tooltip>
							...
							<TooltipContent>{t("ruleRow.agentsTooltip")}</TooltipContent>
						</Tooltip>
					)}
				</span>
				...
				<div className="flex items-center space-x-2 gap-2">
					<Switch
						...
						title={isDisabled ? t("ruleRow.ruleRequiredCannotDisable") : undefined}
					/>
					<Button
						aria-label={isRemote ? t("ruleRow.viewRuleFile") : t("ruleRow.editRuleFile")}
						onClick={handleEditClick}
						size="xs"
						title={isRemote ? t("ruleRow.viewRuleFileReadOnly") : t("ruleRow.editRuleFile")}
						variant="icon">
						{isRemote ? <EyeIcon /> : <PenIcon />}
					</Button>
					<Button
						aria-label={t("ruleRow.deleteRuleFile")}
						disabled={isRemote}
						onClick={handleDeleteClick}
						size="xs"
						title={t("ruleRow.deleteRuleFile")}
						variant="icon">
						<Trash2Icon />
					</Button>
				</div>

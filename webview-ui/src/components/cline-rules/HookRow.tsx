		...
		<div className="mb-2.5">
			<div className="flex items-center px-2 py-4 rounded bg-text-block-background max-h-4">
				...
				<div className="flex items-center space-x-2 gap-2">
					<div title={isWindows ? t("hookRow.windowsToggleTooltip") : undefined}>
						<Switch
							checked={enabled}
							className="mx-1"
							disabled={isWindows}
					...
					<Button
						aria-label={t("hookRow.editHookFile.ariaLabel")}
						onClick={handleEditClick}
						size="xs"
						title={t("hookRow.editHookFile.title")}
						variant="icon">
						<PenIcon />
					</Button>
					<Button
						aria-label={t("hookRow.deleteHookFile.ariaLabel")}
						onClick={handleDeleteClick}
						size="xs"
						title={t("hookRow.deleteHookFile.title")}
						variant="icon">
						<Trash2Icon />
					</Button>
				</div>

			...
			<Alert title={messages.message} variant="danger">
				<AlertDescription className="flex gap-2 justify-end">
					{messages.showDisableButton && (
						<Button aria-label={t("taskHeader.checkpointError.disableCheckpoints")} onClick={handleCheckpointSettingsClick} variant="ghost">
							{t("taskHeader.checkpointError.disableCheckpoints")}
						</Button>
					)}
					{messages.showGitInstructions && (
						<a
							...
							{t("taskHeader.checkpointError.seeInstructions")}
						</a>
					)}
				</AlertDescription>
			</Alert>

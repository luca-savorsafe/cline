...
const ToDoListHeader = memo<{
	...
	const displayText = isCompleted ? t("taskHeader.focusChain.allTasksCompleted") : currentTodo?.text || t("taskHeader.focusChain.todoList")
...
export const FocusChain: React.FC<FocusChainProps> = memo(
	({ currentTaskItemId, lastProgressMessageText }) => {
		...
		return (
			<div
				aria-label={isExpanded ? t("taskHeader.focusChain.collapseFocusChain") : t("taskHeader.focusChain.expandFocusChain")}
				...
				tabIndex={0}
				title={t("taskHeader.focusChain.clickToEditTitle")}>
				...
				{isExpanded && (
					<div className="mx-1 pb-2 px-1 relative" onClick={handleEditClick}>
						...
						{isCompleted && (
							<div className="mt-2 text-xs font-semibold text-muted-foreground">{t("taskHeader.focusChain.newStepsMessage")}</div>
						)}
					</div>
				)}
			</div>

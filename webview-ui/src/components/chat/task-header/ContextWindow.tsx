...
const ConfirmationDialog = memo<{
	...
	return (
		<div className="text-sm my-2 flex items-center gap-0 justify-between">
			<span className="font-semibold text-sm">{t("taskHeader.contextWindow.compactTaskQuestion")}</span>
			<span className="flex gap-1">
				<VSCodeButton
					appearance="secondary"
					className="text-sm"
					onClick={onCancel}
					title={t("taskHeader.contextWindow.cancelTitle")}
					type="button">
					{t("taskHeader.contextWindow.cancel")}
				</VSCodeButton>
				<VSCodeButton
					appearance="primary"
					autoFocus={true}
					className="text-sm"
					onClick={onConfirm}
					title={t("taskHeader.contextWindow.confirmTitle")}
					type="button">
					{t("taskHeader.contextWindow.confirm")}
				</VSCodeButton>
			</span>
		</div>
	)

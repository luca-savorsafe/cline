	...
	return (
		<div className="p-2 border-none rounded-md mb-2 bg-(--vscode-textBlockQuote-background)">
				...
				<div className="mb-3">
					{currentBalance ? (
						<div className="text-foreground">
							{t("chat.creditLimit.currentBalance")} <span className="font-bold">{currentBalance.toFixed(2)}</span>
						</div>
					) : null}
					{totalSpent ? <div className="text-foreground">{t("chat.creditLimit.totalSpent")} {totalSpent.toFixed(2)}</div> : null}
					{totalPromotions ? (
						<div className="text-foreground">{t("chat.creditLimit.totalPromotions")} {totalPromotions.toFixed(2)}</div>
					) : null}
				</div>
			</div>
			...
			<VSCodeButton
				...
				{t("chat.creditLimit.retryRequest")}
			</VSCodeButton>
		</div>
	)
}

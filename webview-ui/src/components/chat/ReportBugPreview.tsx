...
const ReportBugPreview: React.FC<ReportBugPreviewProps> = ({ data }) => {
	...
	return (
		<div className="bg-(--vscode-badge-background) text-(--vscode-badge-foreground) rounded-[3px] p-[14px]">
			<h3 className="font-bold text-base mb-3 mt-0">{bugData.title || t("chat.reportBug.title")}</h3>
			...
			<div className="space-y-3 text-sm">
				...
				{bugData.api_request_output && (
					<div>
						<div className="font-semibold">{t("chat.reportBug.relevantApiRequestOutput")}</div>
				...
				{bugData.provider_and_model && (
					<div>
						<div className="font-semibold">{t("chat.reportBug.providerAndModel")}</div>

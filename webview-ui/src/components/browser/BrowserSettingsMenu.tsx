	...
	return (
		<div ref={containerRef} style={{ position: "relative", marginTop: "-1px", display: "flex" }}>
			<VSCodeButton
				...
				title={t("browser.connection.info")}>
				<i
			...
			{showInfoPopover && (
				...
				<div
					...
					<h4 style={{ margin: "0 0 8px 0" }}>{t("browser.connection.title")}</h4>
					{/* InfoRow - Status row container */}
					<div className="flex flex-wrap whitespace-nowrap mb-1">
						{/* InfoLabel - Fixed-width label */}
						<div className="flex-none w-[90px] font-medium">{t("browser.connection.status")}:</div>
						...
						<div
							...
							{connectionInfo.isConnected ? t("browser.connection.connected") : t("browser.connection.disconnected")}
						</div>
					</div>
					{connectionInfo.isConnected && (
						...
						<div className="flex flex-wrap whitespace-nowrap mb-1">
							{/* InfoLabel - Fixed-width label */}
							<div className="flex-none w-[90px] font-medium">{t("browser.connection.type")}:</div>
							{/* InfoValue - Flexible value container */}
							<div className="flex-1 break-words">{connectionInfo.isRemote ? t("browser.connection.remote") : t("browser.connection.local")}</div>
						</div>
					)}
					{connectionInfo.isConnected && connectionInfo.isRemote && connectionInfo.host && (
						...
						<div className="flex flex-wrap whitespace-nowrap mb-1">
							{/* InfoLabel - Fixed-width label */}
							<div className="flex-none w-[90px] font-medium">{t("browser.connection.remoteHost")}:</div>
							{/* InfoValue - Flexible value container */}

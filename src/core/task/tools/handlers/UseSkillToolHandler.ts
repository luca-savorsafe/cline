...
export class UseSkillToolHandler implements IToolHandler, IPartialBlockHandler {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const skillsManager = config.services.skillsManager
		if (!skillsManager) {
		...
		try {
			const skillContent = await skillsManager.getSkillContent(skillName)

			if (!skillContent) {
				const availableSkills = skillsManager.getAvailableSkills()
